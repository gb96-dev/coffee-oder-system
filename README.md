...
| :--- | :--- | :--- | :--- |
| Menu | 커피 메뉴 목록 조회 | `/menus` | `GET` |
| Stats | 인기 메뉴 조회 | `/menus/popular` | `GET` |

### 👤 유저 & 포인트 관련
| 기능 분류 | 기능명 | API Path | HTTP Method |
| :--- | :--- | :--- | :--- |
| User | 포인트 잔액 조회 | `/users/{userId}/point` | `GET` |
| User | 포인트 충전 | `/users/{userId}/point` | `PATCH` |

### 🛍️ 주문 & 결제 관련
| 기능 분류 | 기능명 | API Path | HTTP Method |
| :--- | :--- | :--- | :--- |
| Order | 커피 주문 및 결제 | `/orders` | `POST` |
...
* **해결**: `created_at` 기준으로 최근 7일 데이터를 필터링한 후, `OrderItem`과 조인하여 메뉴별 판매량을 집계하고 `GROUP BY`와 `SUM`으로 인기 메뉴를 산출합니다. 실제 구현에서는 Spring Data JPA를 활용해 요구사항에 맞는 정확한 통계 데이터를 추출했습니다.
* **선택 이유**: 통계 데이터는 실시간 데이터 정확성이 가장 중요하므로 DB 기반 집계를 우선 선택하였으며, `created_at` 인덱스를 통해 조회 범위를 줄이고 성능을 최적화했습니다.

### 📡 데이터 수집 플랫폼 실시간 전송 전략
* **상황**: 주문 내역을 외부 데이터 수집 플랫폼으로 실시간 전송해야 합니다.
* **해결**: 시스템 복잡도를 낮추기 위해 `MockDataPlatformSender` 컴포넌트를 구현하여 전송 로직을 Mocking 처리했습니다. 
**선택 이유**: 과제 수행 목적에 맞게 불필요한 인프라(Kafka 등) 구축 대신, 비즈니스 로직 완료 시점에 해당 Mock 객체를 호출하고 콘솔에 로그를 출력하는 방식으로 요구사항을 가볍고 직관적으로 충족했습니다.
